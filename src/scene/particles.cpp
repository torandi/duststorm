...
public:
	ParticlesScene(const glm::ivec2 &size)
		: Scene(size)
		, particles(NUM_PARTICLES, new Texture(std::vector<std::string>(1, "textures/fog.png")))
		, camera(75.f, size.x/(float)size.y, 0.1f, 100.f) {
			...
			particles.config.spawn_speed_var = 0.00005f;
			particles.config.avg_ttl = 5.f;
			particles.config.ttl_var = 2.f;
			...
			particles.config.avg_scale_change = 0.1f;
			particles.config.scale_change_var = 0.05f;
			particles.config.avg_rotation_speed = 0.01f;
			particles.config.rotation_speed_var = 0.03f;
			particles.config.birth_color = glm::vec4(1.0, 1.0, 1.0, 0.6);
